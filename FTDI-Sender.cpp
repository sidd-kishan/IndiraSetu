	...
	if (0) { // read from pico
		...
		while (1)
		{
			...
			if ((status == FT_OK) && (TxBytes == 0))
			{
				...
				{
					// FT_Write OK  
					std::cout << "\n Bytes Read " << BytesWritten << " Byte 0x" << byteToHex(TxBuffer[0]) << " 0x" << byteToHex(TxBuffer[1]) << " 0x" << byteToHex(TxBuffer[2]) << " 0x" << byteToHex(TxBuffer[3]) << " last byte 0x" << byteToHex(TxBuffer[255]);
				}
				else
				{
